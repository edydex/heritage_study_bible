...
function setup() {
  ...
  const plugin = { addListener: vi.fn(async (name, callback) => { callbacks.set(name, callback); const remove = vi.fn(async () => {}); removes.push(remove); return { remove } }), command: vi.fn(async () => state) }
