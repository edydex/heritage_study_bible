...
export default function CommunityCalendarPage() {
  ...
  const goHome = () => navigate('/community', { state: authenticated === false && community?.status === 'joined' ? { signInRequired: community.manifest.id } : null })
  useEffect(() => {
  ...
  return <div className="min-h-screen bg-background dark:bg-gray-900 text-gray-900 dark:text-gray-100">
    <header className="bg-primary text-white sticky top-0 z-40 shadow-lg safe-area-top">
      <div className="h-14 px-4 sm:px-6 flex items-center gap-3">
        ...
        <div className="min-w-0"><h1 className="heading-text text-lg font-bold">Calendar</h1><p className="text-xs truncate">{community?.manifest.name}</p></div>
      </div>
    </header>
    <main className="container mx-auto max-w-5xl px-3 sm:px-6 py-5 pb-20">
      {community ? <CalendarBrowser key={community.manifest.id} load={loadCalendar} renderActions={event => <div className="mt-3 flex flex-wrap gap-2">
      ...
      </div>} /> : <p>Save a church in Community Home to open its calendar.</p>}
      {community?.status === 'joined' && authenticated === false && <p className="mt-4 text-sm">To see member events, <button className="underline" onClick={goHome}>check your church sign-in in Community Home</button>.</p>}
