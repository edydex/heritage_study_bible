...
for(const [name,engine] of Object.entries({chromium,firefox})) {
 ...
 const savedBeforeMode=JSON.stringify(saved.project)
 const mode=page.getByRole('checkbox',{name:'Monochrome / colorblind view',exact:true})
 await mode.check()
 await expect(page.locator('.heritage-service-planner__stage')).toHaveAttribute('data-monochrome','true')
 await expect(page.getByRole('button',{name:'Save sermon slides',exact:true})).toBeDisabled()
 await expect(page.getByRole('list',{name:'Used slide colors'})).toContainText('Yellow')
 await expect(page.locator('.heritage-canvas__object[data-object-type="circle"] ellipse').last()).toHaveAttribute('fill',/^url/)
 await expect(text).toHaveCSS('color','rgb(17, 17, 17)')
 await text.evaluate(element=>{element.focus();const range=document.createRange();const node=element.querySelector('span').firstChild;range.setStart(node,0);range.setEnd(node,4);const selection=window.getSelection();selection.removeAllRanges();selection.addRange(range);document.dispatchEvent(new Event('selectionchange'))})
 await page.getByRole('button',{name:'Text color',exact:true}).click()
 await page.getByRole('group',{name:'Text color palette',exact:true}).getByRole('button',{name:'Red · Dots',exact:true}).click()
 await expect(text.locator('span').first()).toHaveCSS('color','rgb(17, 17, 17)')
 await expect(text.locator('span').first()).toHaveCSS('background-image',/data:image\/svg/)
 await mode.uncheck()
 await expect(text.locator('span').first()).toHaveCSS('color','rgb(239, 68, 68)')
 await page.getByRole('button',{name:'Undo',exact:true}).click()
 await page.getByRole('button',{name:'Save sermon slides',exact:true}).click()
 await expect(page.getByRole('button',{name:'Save sermon slides',exact:true})).toBeDisabled()
 assert.deepEqual(saved.project.items,JSON.parse(savedBeforeMode).items)
 await mode.check()
 await page.screenshot({path:`${evidence}/monochrome-${name}.png`,fullPage:true})
 await page.reload()
 if(workspace)await page.getByLabel('Sermon to prepare').selectOption('canvas-rehearsal')
 ...
 results.push({browser:name,saveReopen:true,monochromeLocalOnly:true,namedPatterns:true,monochromeFormatting:true,dragResizeRotation:true,highlight:true,image:true,russianStage:true,workspace,...(workspace ? {compactLayout:true,slideSettingsDialog:true,songHoverFocus:true,passageShortcut:true,ambiguousBooks:true,controlUndo:true,commandUndo:true,nativeTextUndoPreserved:true} : {}),errors})
 await browser.close()
}
