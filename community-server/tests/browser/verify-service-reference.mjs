...
for(const [name,engine] of Object.entries({chromium,firefox})) {
 ...
 const slideButtons=page.locator('.heritage-service-planner__rows li > button')
 await slideButtons.first().click({button:'right'})
 await page.getByRole('menuitem',{name:'Add “Start Translate” Cue',exact:true}).click()
 await slideButtons.nth(1).click({button:'right'})
 await page.getByRole('menuitem',{name:'Add “Stop Translate” Cue',exact:true}).click()
 await page.getByRole('button',{name:'Save sermon slides',exact:true}).click()
 await expect(page.getByRole('button',{name:'Save sermon slides',exact:true})).toBeDisabled()
 const cueTimeline=core.compileServiceProject(saved.project)
 assert.equal(cueTimeline.cues[cueTimeline.cueIds[0]].translationAction,'start')
 assert.equal(cueTimeline.cues[cueTimeline.cueIds[1]].translationAction,'stop')
 await page.screenshot({path:`${evidence}/translation-cues-${name}.png`})
 await page.reload()
 await expect(page.getByRole('button',{name:'Save sermon slides',exact:true})).toBeDisabled()
