...
test('real HTTP calendar privacy, persistent defaults, tag sorting and sermon presentation round trip', { skip: !databaseUrl }, async () => {
  ...
  try {
    ...
    const detail = `community/calendar/events/${events[1]}?date=2026-11-01`
    const publicDetail = await call(detail, 'GET', undefined, false)
    assert.equal(publicDetail.status, 200)
    assert.equal(publicDetail.value.event.startsAt, '2026-11-01T18:00:00.000Z')
    assert.equal(publicDetail.value.event.community, undefined)
    assert.equal((await call(`community/calendar/events/${events[0]}`, 'GET', undefined, false)).status, 404)
    const privateDetail = await call(`community/calendar/events/${events[2]}`)
    assert.equal(privateDetail.status, 200)
    assert.equal(privateDetail.value.event.community, settings.communityId)
    assert.equal((await call(detail.replace('2026-11-01', '2026-11-02'), 'GET', undefined, false)).status, 404)
    const invalidLink = await call(`events/${events[1]}`, 'PATCH', { url: 'test' })
    assert.equal(invalidLink.status, 400, JSON.stringify(invalidLink.value))
    assert.equal(invalidLink.value.errors[0].data.errors[0].path, 'url')
    const pacificAlias = await call(`events/${events[1]}`, 'PATCH', { timeZone: 'PST', url: '' })
    assert.equal(pacificAlias.status, 200)
    assert.equal(pacificAlias.value.doc.timeZone, 'America/Los_Angeles')
