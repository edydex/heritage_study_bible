...
test('legacy visibility flags never expose songs without an exact member receipt', () => {
  const now = new Date('2026-07-25T18:00:00.000Z')
  assert.equal(isSongVisibleToMember({ status: 'published', visibility: 'public' }, now), false)
...
test('legacy English-only songs remain one usable document and empty sync input cannot erase them', () => {
  ...
  const mutation = normalizeSongMutation({ syncDocuments: [], visibility: 'private' }, { existing: song })
