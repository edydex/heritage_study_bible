...
} from './syncshow/SongMemberSharing.ts'
...
export const SYNCSHOW_SERMON_READ_SCOPE = 'syncshow:sermons:read'
...
export function isSongVisibleToMember(song: Record<string, unknown>, now = new Date()) {
  return isSongMemberShareCurrent(song, now)
}
...
export function serializeSongForSync(song: Record<string, unknown>, now = new Date()) {
  return {
    ...
    etag: songEtag(song),
    memberSharing: songMemberSharingSummaryFromSong(song),
  }
}
