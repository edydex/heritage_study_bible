...
export default function PrepareSermonClient() {
  ...
  return <div className="heritage-sermon-workspace">
    <header className="heritage-sermon-workspace__header">
      <details className="heritage-sermon-menu"><summary aria-label="Church workspace menu">☰</summary><nav aria-label="Church workspace">{[...churchWorkspaceLinks, {href:'/admin/collections/events',label:'Events'}].map(link => <a key={link.href} href={link.href}>{link.label}</a>)}</nav></details>
    ...
    {creating ? <form onSubmit={create} className="heritage-sermon-workspace__create">
      ...
      : selected ? <PlanServiceClient key={selected} sermonSyncId={selected} onDirtyChange={setDirty} />
        : <div className="heritage-sermon-workspace__empty"><h2>Build the sermon’s slides here</h2><p>Choose a sermon above, or create one. Add Bible passages, main points, quotations, pictures, and video. Preview English, Russian, and the stage screen as you work.</p><p>After saving, open Plan a service → Sermon → Add whole sermon.</p></div>}
  </div>
}
