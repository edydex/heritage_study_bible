...
import { passageReferenceChoices, type PassageReference } from '../../packages/bible-reference/range.js'
import './passage-reference.css'

export type ResolvedPassage = PassageReference & { bookId: string }
export default function PassageReferenceInput({ books, onResolve, onValidityChange, singleChapter = false, label = 'Passage shortcut' }: {
  books: readonly { id: string; name: string }[]; onResolve: (passage: ResolvedPassage) => void
  onValidityChange?: (valid: boolean) => void; singleChapter?: boolean; label?: string
}) {
  const id = useId()
  const [value, setValue] = useState('')
  const [selected, setSelected] = useState('')
  const choices = passageReferenceChoices(value).map(choice => ({ ...choice,
  ...
  return <div className="heritage-passage-shortcut">
    ...
    <input id={id} type="text" autoComplete="off" placeholder="1 chr 3 7-10" value={value} aria-describedby={`${id}-hint`} onChange={event => {
      ...
      const candidates = passageReferenceChoices(next)
    ...
    <small id={`${id}-hint`} role="status">{current ? `${current.book} ${current.startChapter}:${current.startVerse}${current.endVerse === current.startVerse && current.endChapter === current.startChapter ? '' : `–${current.endChapter !== current.startChapter ? `${current.endChapter}:` : ''}${current.endVerse}`}`
      : !value.trim() ? 'Use Heritage shortcuts, or choose the book and verses below.'
