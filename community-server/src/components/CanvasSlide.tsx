...
export default function CanvasSlide({ objects, mediaUrl, onChange, onImage, uploading = false }: {
  ...
  function patch(change:ObjectValue) { if(selected)commit(current.current.map(object=>object.id===selected.id ? {...object,...change} : object)) }
  function frame(change:ObjectValue) {
    if(!selected)return
    const next={...(current.current.find(value=>value.id===selected.id)?.frame || selected.frame),...change}
  ...
  function begin(event:React.PointerEvent,object:ObjectValue,kind:string) {
    ...
    object = current.current.find(value=>value.id===object.id) || object
  ...
  function add(type:string,extra:ObjectValue={}) {const value=newCanvasObject(type,extra);commit([...current.current,value]);setSelectedId(value.id)}
  function layer(direction:number) {
    if(!selected)return
    const next=[...current.current], index=next.findIndex(value=>value.id===selected.id),target=clamp(index+direction,0,next.length-1)
