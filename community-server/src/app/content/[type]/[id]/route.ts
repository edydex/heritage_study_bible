...
import { loadActivePublicSermonPublication } from '@/lib/syncshow/SermonPublicationStore'
...
export async function GET(request: Request, context: { params: Promise<{ type: string; id: string }> }) {
  ...
  if (type === 'sermons' && /^sermon-[a-f0-9]{64}$/.test(id)) {
    try {
      ...
      if (!publication) return unavailablePublicSermonResponse()
      return publicSermonSourceResponse(
        publication.detailSource,
        PUBLIC_SERMON_DETAIL_MEDIA_TYPE,
        publication.detailChecksum,
      )
    } catch {
      // A strict public ID can never fall through to the legacy numeric route.
  ...
  try {
    ...
    const projectedContent = type === 'songs'
      ? memberSongContentProjection(
