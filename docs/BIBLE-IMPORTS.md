...
SyncShow's matching expansion adds separate Scripture credits to audience/stage native scenes and raster previews. Update SyncShow before making new credited packages; older strict scene readers reject the added credit field. Existing uncredited packages remain readable in the updated app. Standalone desktop module installation is included in SyncShow Preview 31: Prepare → This computer → Scripture → Bible translations. Its native file picker previews a digest-checked snapshot before explicit installation. Full editions remain private to that computer; selected text and credits travel with offline packages. The seven verified owner-test installers are in the private heritage-preview-builds repository. See the integration repository’s current STATUS for source pins and testing instructions; installed server state is checked separately through the supported server tools.
