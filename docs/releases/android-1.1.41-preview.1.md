...
Publication recovery: the preceding 1.1.40 build passed native acceptance but
failed before publishing any release assets. The publisher now resolves only
the commit SHA, so large source updates cannot overflow its response buffer.
This package uses a new immutable release identity.
