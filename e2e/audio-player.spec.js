...
test('phone player and Internal Storage remain usable without horizontal overflow', async ({ page }) => {
  ...
  const nav = page.locator('.reader-bottom-nav')
  await expect(nav).toBeVisible()
  await expect.poll(async () => (await nav.boundingBox()).y + (await nav.boundingBox()).height - (await player.boundingBox()).y).toBeLessThanOrEqual(1)
  await player.locator('.audio-player-title').click()
  await player.getByRole('button', { name: 'Audio library', exact: true }).click()
