...
test('first reference search and first saved note click land at the verse',async({page})=>{
  await page.addInitScript(()=>localStorage.setItem('bible-study-notes',JSON.stringify([{id:'jump-note',book:'Luke',chapter:12,verse:34,text:'First note navigation',dateCreated:'2026-09-16T00:00:00Z'}])))
  await openReader(page)
  await submitSearch(page,'Joh 3:16')
  await expect(page).toHaveURL(/#\/john\/3/)
  await expect(page.locator('#verse-3-16')).toBeInViewport()
  await page.waitForTimeout(350)
  await expect(page.locator('#verse-3-16')).toBeInViewport()
  await page.getByTestId('open-bookmarks').click()
  await page.getByText('Luke 12:34',{exact:true}).click()
  await page.getByText('Note on Luke 12:34',{exact:true}).click()
  await expect(page).toHaveURL(/#\/luke\/12/)
  await expect(page.locator('#verse-12-34')).toBeInViewport()
  await page.waitForTimeout(350)
  await expect(page.locator('#verse-12-34')).toBeInViewport()
})

test('ambiguous book prefixes offer clickable alternatives and clear invalid chapters',async({page})=>{
  await openReader(page)
  await submitSearch(page,'Jo 3:16')
  const chooser=page.getByRole('dialog',{name:'Which book did you mean?'})
  await expect(chooser.getByRole('option',{name:/John 3:16/})).toBeVisible()
  await expect(chooser.getByRole('option',{name:/Job 3:16/})).toBeVisible()
  await chooser.getByRole('option',{name:/John 3:16/}).click()
  await expect(page.locator('#verse-3-16')).toBeInViewport()
  await submitSearch(page,'Ma 5')
  await expect(chooser.getByRole('option',{name:/Malachi/})).toHaveAttribute('aria-disabled','true')
  await chooser.getByRole('option',{name:/Matthew 5/}).click()
  await expect(page).toHaveURL(/#\/matthew\/5/)
  await submitSearch(page,'Luk 12:34')
  await expect(page.locator('#verse-12-34')).toBeInViewport()
})
